    ...
    return (
        <>
            ...
            <div className={`${lang === "fa" ? `${VazirFont.className}` : ""} ${generateClass("boxIn")}`}>
                ...
                <div className={`${styles.rectangleList} ${styles.rectangleListIn}`}>
                    {
                        designs.map((design: IDesign, index: number) => (
                            <Link href={`/${lang}/portfolio/designs/${design.key}`} key={index} className={`${styles.rectangleItem} ${styles.rectangleItemIn}`}>
                ...
                <Link href={`/${lang}/portfolio`} className={`${lang === "fa" ? "fakharnia-arrow-right" : "fakharnia-arrow-left"} ${generateClass("backButton")}`}></Link>
            </div>
        </>
    )
}
