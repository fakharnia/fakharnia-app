                    ...
                    <div className={styles.nvaList}>
                        {
                            designPagination().map((dsg: IDesign, index: number) => (
                                <Link
                                    key={index}
                                    href={`/${lang}/portfolio/designs/${dsg.key}`}
                                    className={`${styles.navItem} ${isActive(dsg) ? styles.navItemActive : ""}`}></Link>
                            ))
                        }
                    </div>
                </div>
                <Link href={`/${lang}/portfolio/designs`} className={`${lang === "fa" ? `fakharnia-arrow-right` : "fakharnia-arrow-left"} ${generateClass("backButton")}`}></Link>
            </div >
        </>
    )
