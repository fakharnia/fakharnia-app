...
import { getProjects } from "@/lib/portfolio.lib";
    ...
    return (
        <>
            ...
            <div className={`${lang === "fa" ? `${styles.farsiLang} ${VazirFont.className}` : ""} ${styles.boxIn}`}>
                ...
                <div className={`${styles.squareList}  ${styles.squareListIn}`}>
                    {
                        projects.map((project: IProject, index: number) => (
                            <Link href={`/${lang}/portfolio/projects/${project.key}`} key={index} className={`${styles.squareItem} ${styles.squareItemIn}`}>
                ...
                <Link href={`/${lang}/portfolio`} className={`${lang === "fa" ? `fakharnia-arrow-right ${styles.backButtonFa}` : "fakharnia-arrow-left"} ${styles.backButton}`}></Link>
            </div>
        </>
    )
}
