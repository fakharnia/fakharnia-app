...
const Page = async (props: propType) => {
    ...
    return (
        <>
                    ...
                    <div className={styles.footerNav}>
                        {
                            projectPagination().map((prj: IProject, index: number) => (
                                <Link key={index} className={`${styles.navItem} ${isActive(prj) ? styles.navItemActive : ""}`} href={`/${lang}/portfolio/projects/${prj.key}`} >
            ...
            <Link href={`/${lang}/portfolio/projects`} className={`${lang === "fa" ? `fakharnia-arrow-right ${styles.backButtonFa}` : "fakharnia-arrow-left"} ${styles.backButton}`}></Link>
        </>
    )
}
export default Page;