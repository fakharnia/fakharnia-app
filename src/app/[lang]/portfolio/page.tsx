            ...
            <div className={styles.container}>
                    ...
                    <div className={styles.squareList}>
                        {
                            projects.map((project: IProject, index: number) => (
                                <Link href={`/${lang}/portfolio/projects/${project.key}`} key={index} className={styles.squareItem}>
                                    <Logo project={project} classes={styles.squareImage} />
                                </Link>))
                        }
                    </div>
                    <Link href={`/${lang}/portfolio/projects`} className={`${styles.moreButton} ${lang === "fa" ? styles.moreButtonFa : ""}`}>{portfolio.moreButton}</Link>
                </div>
                <div className={`${lang === "fa" ? `${styles.farsiLang} ${VazirFont.className}` : ""} ${styles.box}`}>
                    ...
                    <div className={styles.rectangleList}>
                        {
                            designs.map((design: IDesign, index: number) => (
                                <Link href={`/${lang}/portfolio/designs/${design.key}`} key={index} className={styles.rectangleItem}>
                                    <Image className={styles.rectangleImage} src={`${URL}/design/${design.coverUrl}`} alt={design.coverAlt} width={300} height={150}></Image>
                                </Link>))
                        }

                    </div>
                    <Link href={`/${lang}/portfolio/designs`} className={`${styles.moreButton} ${lang === "fa" ? styles.moreButtonFa : ""}`}>{portfolio.moreButton}</Link>
                </div>
                <div className={`${lang === "fa" ? `${styles.farsiLang} ${VazirFont.className}` : ""} ${styles.downloadBox}`}>
                    ...
                    <div className={styles.dowloadButons}>
                        <Link href={`/${lang}/portfolio/resume`} className={`fakharnia-more ${styles.downloadButton} ${lang === "fa" ? styles.farsiIconButton : ""}`}></Link>
                    </div>
                </div>
            </div>
        </>
