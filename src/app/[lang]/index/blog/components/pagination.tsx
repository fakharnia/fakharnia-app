...
export const Pagination = (props: paginationPropType) => {
    ...
    const onChangePage = (page: number, event: MouseEvent<HTMLButtonElement>): void => {
        router.push(`/${lang}/index/blog?page=${page}&perPage=${params.perPage}${getRestParams()}`);
    }
    ...
    const onChangePerpage = (number: number, event: MouseEvent<HTMLButtonElement>): void => {
        ...
        if (number > pagesOnThisNumber) {
            router.push(`/${lang}/index/blog?page=${pagesOnThisNumber}&perPage=${number}${getRestParams()}`);
        } else {
            router.push(`/${lang}/index/blog?page=${params.page}&perPage=${number}${getRestParams()}`);
        }
