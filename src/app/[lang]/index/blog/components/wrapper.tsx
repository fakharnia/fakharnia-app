...
export const Wrapper = (props: wrapperPropType) => {
    ...
    const onSelectTag = (tag: string, event: ChangeEvent<HTMLInputElement>) => {
        ...
        router.push(`/${lang}/index/blog?${prepareFilter("tags", updatedTags)}`);
    ...
    const onChangeSort = (sort: string, event: ChangeEvent<HTMLInputElement>) => {
        setSort(sort);
        setSortMenu(false);
        router.push(`/${lang}/index/blog?${prepareFilter("sort", sort)}`);
        document.getElementById("content")?.scrollTo({
            top: 0, left: 0,
            behavior: "smooth"
        });
    ...
    const onChangeSortFlow = (sortFlow: string, event: MouseEvent<HTMLButtonElement>) => {
        ...
        router.push(`/${lang}/index/blog?${prepareFilter("sortFlow", sortFlow)}`);
        document.getElementById("content")?.scrollTo({
            top: 0, left: 0,
            behavior: "smooth"
        });
    ...
    const onFormSubmitted = (event: FormEvent<HTMLFormElement>) => {
        ...
        router.push(`/${lang}/index/blog?${prepareFilter("", undefined)}`);
        document.getElementById("content")?.scrollTo({
            top: 0, left: 0,
            behavior: "smooth"
        });
