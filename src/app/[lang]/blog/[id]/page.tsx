...
export async function generateMetadata(
    ...
    const url = `https://fakharnia.com/${lang}/blog/${id}`;
                ...
                <ul className={styles.tagList}>
                    {
                        post.tags.map((tag: string, index: number) => (
                            ...
                            <Link href={`/${lang}/blog?page=1&perPage=5&tags=${tag}`} key={index} className={`${getClasses("tag")} ${GenosFont.className}`}>{tag}</Link>
                        ))
                    }
                </ul>
                <Link href={`/${lang}/blog?${getCurrentLink()}`} className={`${lang === "fa" ? `fakharnia-arrow-right ${styles.backButtonFa}` : "fakharnia-arrow-left"} ${styles.backButton}`}></Link>
            </div>
        </>
    )
}
