                    ...
                    posts?.map((post: IPost, index: number) => (
                        <div className={`${styles.post} ${language === "fa" ? VazirFont.className : ""}`} key={index}>
                                ...
                                <div className={styles.postTags}>
                                    {
                                        post?.tags?.map((tag: string, index: number) => (
                                            <Link href={`/${language}/blog?page=1&perPage=5&tags=${tag}`} key={index} className={`${genClass("postTag")} ${GenosFont.className}`}>{tag}</Link>
                                        ))
                                    }
                                </div>
                            </div>
                            <div className={styles.postOptions}>
                                ...
                                <Link href={`/${language}/blog/${post.key}?page=1&perPage=5`} className={genClass("postReadButton")}>{blog.read}</Link>
                            </div>
                        </div>
